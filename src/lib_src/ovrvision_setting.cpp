...
namespace OVR
{
  ...
  void OvrvisionSetting::InitValue()
  {
    ...
    m_rightCameraDistortion = (cv::Mat_<double>(1, 8) << -3.0493990540623439e-001, 1.3662653080461551e-001, 1.0423167776015031e-003, 2.8491358735884113e-003, 5.9120577005421594e-002, 3.3803762878286243e-002, -5.1376486225192128e-002, 1.5379685303460996e-001);
    m_R1 = (cv::Mat_<double>(3, 3) << 9.9989337632971764e-001, -1.3329319960629154e-002, -5.9636567091731069e-003, 1.3333406344070171e-002, 9.9991089752301443e-001, 6.4597986305001164e-004, 5.9545148603057114e-003, -7.2542684450603058e-004, 9.9998200859249042e-001);
    m_R2 = (cv::Mat_<double>(3, 3) << 9.9977730126048470e-001, -1.2839202046508414e-002, -1.6748217072606403e-002, 1.2827713773375438e-002, 9.9991740689173758e-001, -7.9319244304950341e-004, 1.6757017743338598e-002, 5.7817446527048781e-004, 9.9985942415453444e-001);
    m_trans = (cv::Mat_<double>(1, 3) << -6.1249914523852240e+001, 7.5758816805225948e-001, 1.6870717314641153e+000);
    m_focalPoint = (cv::Mat_<float>(1, 1) << 428.0f);
  }
...
void OvrvisionSetting::GetUndistortionMatrix(Cameye eye, ovMat& mapX, ovMat& mapY, int width, int height)
{
  ...
  if (eye == OV_CAMEYE_LEFT)
  {
    ...
    // Undistort
    initUndistortRectifyMap(left_CamIns, m_leftCameraDistortion, m_R1, camPs, size, CV_32FC1, mapX, mapY);
  }
  else
  {
    ...
    // Undistort
    initUndistortRectifyMap(right_CamIns, m_rightCameraDistortion, m_R2, camPs, size, CV_32FC1, mapX, mapY);
  }
}
};