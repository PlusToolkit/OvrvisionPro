...
int OvrvisionPro::Open(int locationID, OVR::Camprop prop, const char *pVendorName, int deviceType, void *pDevice)
{
    ...
    {
#if defined(WIN32)
      ::MessageBox(NULL, TEXT("OvrvisionPro SDK requires a GPU with OpenCL1.2+ support."), TEXT("OpenCL Error!"), MB_ICONERROR | MB_OK);
#elif defined(MACOSX)
      NSLog(@"OvrvisionPro SDK requires a GPU with OpenCL1.2 + support.");
#elif defined(LINUX)
      std::cout << "OvrvisionPro SDK requires a GPU with OpenCL1.2 + support." << std::endl;
#endif
      return 0;
    }
    // Opened
  ...
  // Capture frame
  bool OvrvisionPro::Capture(OVR::Camqt qt)
  {
    if (!m_isOpen)
    {
      return false;
...
#endif
    {
      ...
      {
        m_pOpenCL->DemosaicRemap(m_pFrame);
        return true;
      }
      else if (qt == OV_CAMQT_DMS)
      {
        m_pOpenCL->Demosaic(m_pFrame);
        return true;
      }
    }
    return false;
  ...
  // Get color histogram in HSV space
  ...
  cl_mem OvrvisionPro::GetLeftCLImage()
  {
    return m_pOpenCL->GetLeftCLImage();
  }

  cl_mem OvrvisionPro::GetRightCLImage()
  {
    return m_pOpenCL->GetRightCLImage();
  }

  cl_context OvrvisionPro::GetContext()
  {
    return m_pOpenCL->GetContext();
  }

  cl_device_id OvrvisionPro::GetDeviceId()
  {
    return m_pOpenCL->GetDeviceId();
  }

  cl_platform_id OvrvisionPro::GetPlatformId()
  {
    return m_pOpenCL->GetPlatformId();
  }
