...
namespace
{
  static const double NEGLIGIBLE_DIFFERENCE = 0.001;
}
...
#define MSS_TEMPSIZE  (8)
  ...
  // GetSrcFilterFromID
  IBaseFilter* OvrvisionDirectShow::GetSrcFilterFromID(ICreateDevEnum* pDev, usb_id vid, usb_id pid, int skip)
  {
    HRESULT hr;
    IBaseFilter* filter = NULL;
    IEnumMoniker* pClassEnum = NULL;

    // USB Camera capture Filter
...
#define MAX_CATEGORYNUM (10)
  ...
  // Create device
  int OvrvisionDirectShow::CreateDevice(usb_id vid, usb_id pid, int cam_w, int cam_h, double rate, int skip)
  {
    // Local var
    HRESULT hr;
    ...
    // Get DestFilter
    hr = CoCreateInstance(CLSID_NullRenderer, NULL, CLSCTX_INPROC_SERVER, IID_IBaseFilter, (void**)&m_pDestFilter);
    if (FAILED(hr))
    {
      DeleteDevice(); // Delete
      return RESULT_FAILED;
    ...
    if (SUCCEEDED(hr) && m_pGraph != NULL)
    {
      ...
      // Acquire in advance the camera control pointer.
      if (FAILED(m_pSrcFilter->QueryInterface(IID_IAMVideoProcAmp, (void**)&m_pAMVideoProcAmp)))
      {
        DeleteDevice();
        return RESULT_FAILED;
      }
      if (FAILED(m_pSrcFilter->QueryInterface(IID_IAMCameraControl, (void**)&m_pIAMCameraControl)))
      {
        DeleteDevice();
        return RESULT_FAILED;
      }
  ...
  int OvrvisionDirectShow::GetBayer16Image(unsigned char* pImage, bool nonblocking)
  {
    ...
    if (pImage)
    {
      ...
      memcpy(pImage, m_pSGCallback->m_pPixels, m_pSGCallback->m_LatestBufferLength);
  ...
  */
  ...
  int OvrvisionDirectShow::SetCameraSetting(CamSetting proc, int value, bool automode)
  {
    ...
    switch (proc)
    {
      case OV_CAMSET_EXPOSURE:
        m_pAMVideoProcAmp->Set(VideoProcAmp_Brightness, value, autoflag);
        break;
      case OV_CAMSET_GAIN:
        m_pAMVideoProcAmp->Set(VideoProcAmp_Gain, value, autoflag);
        break;
      case OV_CAMSET_WHITEBALANCER:
        m_pAMVideoProcAmp->Set(VideoProcAmp_Sharpness, value, autoflag);
        break;
      case OV_CAMSET_WHITEBALANCEG:
        m_pAMVideoProcAmp->Set(VideoProcAmp_Gamma, value, autoflag);
        break;
      case OV_CAMSET_WHITEBALANCEB:
        m_pAMVideoProcAmp->Set(VideoProcAmp_WhiteBalance, value, autoflag);
        break;
      case OV_CAMSET_BLC:
        m_pAMVideoProcAmp->Set(VideoProcAmp_BacklightCompensation, value, autoflag);
        break;
      case OV_CAMSET_DATA:
        m_pAMVideoProcAmp->Set(VideoProcAmp_Contrast, value, false);
        break;
      default:
        return RESULT_FAILED;
        break;
  ...
  int OvrvisionDirectShow::GetCameraSetting(CamSetting proc, int* value, bool* automode)
  {
    ...
    {
      case OV_CAMSET_EXPOSURE:
        m_pAMVideoProcAmp->Get(VideoProcAmp_Brightness, (long*)value, &autoflag);
        break;
      case OV_CAMSET_GAIN:
        m_pAMVideoProcAmp->Get(VideoProcAmp_Gain, (long*)value, &autoflag);
        break;
      case OV_CAMSET_WHITEBALANCER:
        m_pAMVideoProcAmp->Get(VideoProcAmp_Sharpness, (long*)value, &autoflag);
        break;
      case OV_CAMSET_WHITEBALANCEG:
        m_pAMVideoProcAmp->Get(VideoProcAmp_Gamma, (long*)value, &autoflag);
        break;
      case OV_CAMSET_WHITEBALANCEB:
        m_pAMVideoProcAmp->Get(VideoProcAmp_WhiteBalance, (long*)value, &autoflag);
        break;
      case OV_CAMSET_BLC:
        m_pAMVideoProcAmp->Get(VideoProcAmp_BacklightCompensation, (long*)value, &autoflag);
        break;
      case OV_CAMSET_DATA:
        m_pAMVideoProcAmp->Get(VideoProcAmp_Contrast, (long*)value, &autoflag);
        break;
      default:
        return RESULT_FAILED;
        break;
