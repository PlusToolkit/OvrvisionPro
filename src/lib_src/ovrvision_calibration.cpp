...
namespace OVR
{
  ...
  bool OvrvisionCalibration::FindChessBoardCorners(const unsigned char* left_img, const unsigned char* right_img)
  {
    ...
    cv::cvtColor(src_left, src_gray_left, cv::COLOR_BGR2GRAY);
    cv::cvtColor(src_right, src_gray_right, cv::COLOR_BGR2GRAY);
  ...
  int OvrvisionCalibration::GetImageCount() const
  {
  ...
  void OvrvisionCalibration::SaveCalibrationParameter(OvrvisionPro* system, bool param_output)
  {
    ...
    if (param_output)
    {
      cv::FileStorage cvfs("./ovrvision_param_output.xml", cv::FileStorage::WRITE | cv::FileStorage::FORMAT_XML);
